...
// Phase 2: Discover which years currently exist
...
async function getEventGroups(page, years) {
  ...
  // De-duplicate: the same titleId can appear more than once in the DOM
  const seen = new Map();
  for (const g of groups) {
    if (!seen.has(g.titleId)) seen.set(g.titleId, g);
  }
  const uniqueGroups = [...seen.values()];

  console.log(`Found ${groups.length} raw sub-groups, ${uniqueGroups.length} unique.`);
  return uniqueGroups;
}
...
function saveResults(records) {
  ...
  console.log(`\nSaved ${records.length} records to ins_events.json`);
}
